...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      {/* Backdrop Gelap Khusus Layar HP saat Sidebar Aktif */}
      {isOpen && (
        <div 
      ...
      {/* Sidebar Container */}
      {/* Sembunyi ke kiri di HP (-translate-x-full) & melayang (z-50) jika isOpen aktif */}
      <div className={`fixed top-0 left-0 w-72 h-screen bg-[#05040a] text-slate-400 flex flex-col justify-between border-r border-slate-900 shadow-xl select-none z-50 transition-transform duration-300 ease-in-out
        ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        
        {/* BAGIAN ATAS: LOGO BRAND */}
        <div>
          <div className="flex items-center justify-between px-6 py-6 border-b border-slate-900 font-sans">
            ...
            <button 
              onClick={() => setIsOpen(false)}
              className="md:hidden p-1.5 text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors"
            >
              <FaTimes size={18} />
            </button>
          </div>
          ...
          <nav className="mt-6 px-3 space-y-1.5">
            {menuItems.map((item) => (
              <NavLink
                key={item.path}
                to={item.path}
                onClick={() => setIsOpen(false)} // Otomatis menutup sidebar di HP setelah menu ditekan
                className={({ isActive }) =>
